                            ...
                            <tr>
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <div class="relative inline-block text-left">
                                            ...
                                            <div class="dropdown-content absolute right-0 mt-2 py-2 w-48 bg-white rounded-md shadow-lg">
                                                <a href="/adminSuratpesanan?tambahdata_id=<?= $document['id'] ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Surat Pesanan</a>
                                                <a href="/adminUmk?tambahdata_id=<?= $document['id'] ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">UMK</a>
                                                <a href="/adminSpph?tambahdata_id=<?= $document['id'] ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">SPPH</a>
                                                <a href="/adminKontrak?tambahdata_id=<?= $document['id'] ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Kontrak</a>
                                ...
                                <?php if ($document['status_selesai'] == 'diterima') : ?>
                                    <td class="border p-3 text-center">
                                        <button id="update-status-btn-<?= $document['id'] ?>" class="bg-green-500 text-white py-1 px-3 rounded cursor-pointer open-modal-btn" data-id="<?= $document['id'] ?>">Selesai</button>
                                    </td>
                                <?php else : ?>
                                    <td class="border p-3 text-center">
                                        <button id="update-status-btn-<?= $document['id'] ?>" class="bg-yellow-500 text-white py-1 px-3 rounded cursor-pointer open-modal-btn" data-id="<?= $document['id'] ?>">Update Status</button>
                                    </td>

                                <?php endif; ?>
                                </td>
                                <td class="border p-3 text-center relative">
                                    <button class=" bg-red-500 text-white border-none py-1 px-3 rounded cursor-pointer" data-id="<?= $document['id'] ?>" onclick="confirmDeleteDocument(this)">Hapus</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
...
`);
        ...
        // Fungsi untuk menandai proses selesai dan mengubah warna tombol
        function completeProcess() {
            // Mendapatkan ID dokumen dari input tersembunyi
            const id = document.getElementById('documentId').value;

            // Mengambil status_selesai dari form
            const statusSelesai = document.getElementById('status_selesai').value;

            // Kirim permintaan POST untuk memperbarui status di server
            fetch(`/admin/updateStatus/${id}`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Mengubah warna tombol update status menjadi hijau jika status_selesai = 'dikirim'
                        const statusButton = document.getElementById(`update-status-btn-${id}`);
                        if (statusButton && statusSelesai === 'dikirim') {
                            statusButton.classList.remove('bg-yellow-500');
                            statusButton.classList.add('bg-green-500');
                            statusButton.textContent = 'Selesai';
                        }
                    } else {
        ...
        function closeUpdateStatusModal() {
            document.getElementById('UpdateStatusModal').style.display = 'none'; // Sembunyikan modal konfirmasi
        }
